...
Go checking core (`go_core/main.go`) + GitHub Actions CI + static website (HTML/CSS/JS in `config/web/*`).

The current phase is **Website Overhaul** — improving the user-facing site. Context is in
`AUDIT.md` §9 (website audit). Read the relevant `notes` section of each story for full context.
...
3. Ensure you are on the branch from PRD `branchName` (`ralph/website-overhaul`). If not, check it
...
- **YAML/JSON touched:** validate it loads (`python -c "import yaml; yaml.safe_load(open('<file>'))"`
  or `jq . <file>`).
- **HTML/CSS/JS:** open the generated or source files and verify visually where possible;
  at minimum validate JSON/YAML configs and Python compilation.
...
- **Learnings for future iterations:**
  - Patterns discovered (e.g., "merge.py build_html() injects CSS/JS inline via placeholders")
  - Gotchas (e.g., "template.html uses {{PLACEHOLDER}} syntax, not Handlebars")
  - Useful context (e.g., "vercel.json rewrites /sub/* to .txt files")
---
```
...
- **Website:** `merge.py:build_html()` reads `config/web/template.html` and injects CSS/JS inline.
  Placeholders use `{{UPPERCASE}}` syntax. The generated `index.html` is published to `gh-pages`.
  `vercel.json` handles rewrite rules. No external JS dependencies beyond Font Awesome + Google Fonts.
...
- For website changes: test by opening `index.html` in a browser when possible.
