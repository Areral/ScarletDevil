...
class BatchEngine:
    ...
    async def _ping_phase(self, node: ProxyNode, port: int) -> dict:
    ...
    async def check_batch(self, nodes: List[ProxyNode], is_champion: bool = False, batch_num: int = 0) -> List[ProxyNode]:
        ...
        try:
            ...
            async def run_phases():
                ...
                for chunk in ping_chunks:
                    chunk_tasks =[self._ping_phase(n, p) for n, p in chunk]
...
class Inspector:
    ...
    async def _l4_check(self, node: ProxyNode) -> Optional[ProxyNode]:
        ...
        try:
            ...
            fut = loop.getaddrinfo(host, port, family=socket.AF_UNSPEC, type=socket.SOCK_STREAM)
            task = asyncio.ensure_future(fut)
            try:
                addr_info = await asyncio.wait_for(task, timeout=2.0)
            except asyncio.TimeoutError:
                task.add_done_callback(_safe_exc)
                return None
            except Exception:
                return None
        ...
        try:
            fut_conn = asyncio.open_connection(ip_str, port)
            _, writer = await asyncio.wait_for(fut_conn, timeout=2.0)
            writer.close()
            try: 
                await writer.wait_closed()
            except Exception: 
    ...
    async def process_all(self, nodes: List[ProxyNode]) -> List[ProxyNode]:
        ...
        l4_chunks = _chunk_list(nodes, 50)
        valid_nodes =[]
        
        processed_l4 = 0
        for chunk in l4_chunks:
            res = await asyncio.gather(*(self._l4_check(n) for n in chunk), return_exceptions=True)
            valid_nodes.extend([n for n in res if isinstance(n, ProxyNode)])
            processed_l4 += len(chunk)
            
            if processed_l4 % 10000 < 50:
        ...
        for i in range(0, total, batch_size):
            ...
            res = await self._process_batch(batch, batch_num, total_batches)
