...
class Exporter:
    ...
    def _xray_encode_value(key: str, value: str) -> str:
        k = key.lower()
        if k in ("pbk", "sid", "flow", "fp", "type", "security", "encryption",
                 "net", "tls", "protocol"):
            return value
        if k == "alpn":
        ...
        if k == "path":
            return urllib.parse.quote(value, safe="")
        if k in ("host", "sni", "peer", "add", "server"):
    ...
    def _urlencode(query_dict: Dict[str, Any]) -> str:
        ...
        for k, raw_v in query_dict.items():
            if raw_v is None:
                continue
            if (str(raw_v).lower() in ("none", "null", "")
                    and k.lower() not in ("sid", "pbk", "path", "spx")):
                continue
            v = ("1" if isinstance(raw_v, bool) and raw_v
                 else ("0" if isinstance(raw_v, bool) else str(raw_v)))
    ...
    def _build_url(node: ProxyNode, name: str) -> str:
        ...
        try:
            if node.protocol == "vless":
                ...
                allow_insecure = any(
                    k.lower() in ("allowinsecure", "insecure")
                    and str(v).lower() in ("1", "true", "yes")
                ...
                data: Dict[str, Any] = {
                    ...
                    "type": "none",
                    "tls": "tls" if c.security in ("tls", "reality") else "",
                }
                if c.host:
                    data["host"] = c.host
                if c.path:
                    data["path"] = c.path
                if c.type == "grpc" and c.service_name:
                    data["path"] = c.service_name
                if data["tls"]:
                    if c.sni:
                        data["sni"] = c.sni
                    if c.fp:
                ...
                b64_up = Exporter._safe_b64encode(
                    f"{c.method}:{c.password}".encode("utf-8")
                )
                q = dict(c.raw_meta or {})
                ...
                if c.obfs:
                    q["obfs"] = c.obfs
                if c.obfs_password:
                    q["obfs-password"] = c.obfs_password

                insecure_val = c.raw_meta.get("insecure", "1")
                if str(insecure_val).lower() in ("1", "true", "yes"):
                    q["insecure"] = "1"

                return (
                    f"hysteria2://{urllib.parse.quote(c.password, safe='')}@"
                    f"{host_for_uri}:{port}?{Exporter._urlencode(q)}#{encoded_name}"
                )
    ...
    def generate_subscription(nodes: List[ProxyNode], title: str) -> str:
        logger.debug(
            f"Exporter: Генерация тела подписки '{title}' для {len(nodes)} узлов"
        )
    ...
    def save_files(
        ...
        for filename, node_list, title in [
            ...
            try:
                ...
                logger.debug(
                    f"Exporter: Файл {filename} успешно сохранён ({len(node_list)} узлов)"
                )
            except Exception as e:
